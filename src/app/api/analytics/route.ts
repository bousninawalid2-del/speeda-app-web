...
import { deriveMosFactors } from '@/lib/mos-score';
...
export async function GET(req: NextRequest) {
  ...
  // ── 4b. MOS Score factors (weighted split of the same MOS source) ────────
  const factors = deriveMosFactors(mosScore);

  // ── 4c. Score history (same MOS source-of-truth; latest point = mosScore) ─
  const now = Date.now();
  ...
  for (let w = 3; w >= 0; w--) {
    ...
    const weekScore =
      ...
      (userActivity ? 30 : 0);
    // Keep the latest visible point exactly aligned with the current MOS block value.
    scoreHistory.push({ week: `Week ${4 - w}`, score: w === 0 ? mosScore : weekScore });
  }
