...
export const TopUpScreen = ({ onBack }: TopUpScreenProps) => {
  ...
  const handlePurchase = async () => {
    if (isCustom) {
      toast.error(t('topUp.toasts.customError'));
      return;
    }
    if (!selectedPkg) {
      toast.error(t('topUp.toasts.selectPackage'));
      return;
    }
    try {
      ...
      toast.error(err instanceof Error ? err.message : t('topUp.toasts.failed'));
    }
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="bg-background min-h-screen pb-8">
      <div className="px-5 pt-6">
        ...
        <h3 className="text-[16px] font-bold text-foreground mb-3">{t('topUp.selectPackage')}</h3>
        <div className="flex flex-wrap gap-2 mb-2">
          {(packages ?? []).map(pkg => (
            <button key={pkg.id} onClick={() => { setSelectedPkg(pkg); setIsCustom(false); }}
              ...
              {t('topUp.packageLabel', { count: pkg.tokenCount, price: pkg.price.toLocaleString() })}
            </button>
          ))}
        ...
        >
          ...
          {t('topUp.continuePayment', { amount: displayAmount.toLocaleString() })}
        </button>
