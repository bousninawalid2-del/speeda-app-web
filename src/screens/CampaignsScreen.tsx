...
export const CampaignsScreen = ({ onNavigate, externalCampaigns, externalStats, isLoading, onUpdateCampaign }: CampaignsScreenProps) => {
  ...
  // Use external data when available, fall back to mock
  const displayCampaigns = externalCampaigns ?? campaigns;
              ...
              <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} onClick={e => e.stopPropagation()} className="bg-card rounded-2xl p-6 w-full max-w-[360px]">
                ...
                <button onClick={async () => {
                  ...
                  if (onUpdateCampaign) {
                    ...
                    toast.success(`Budget increased by SAR ${newBudget}`);
                    setShowCampaignBudgetModal(false);
                  }
                }} className="w-full h-11 rounded-xl gradient-btn text-primary-foreground text-[13px] font-bold btn-press mt-4 disabled:opacity-50">Confirm</button>
              </motion.div>
            </motion.div>
          )}
        </AnimatePresence>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-foreground/40 flex items-center justify-center px-6" onClick={() => setShowStopConfirm(false)}>
              <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} onClick={e => e.stopPropagation()} className="bg-card rounded-2xl p-6 w-full max-w-[360px] text-center">
                ...
                <div className="flex gap-2">
                  ...
                  }} className="flex-1 h-11 rounded-xl bg-destructive text-primary-foreground text-[13px] font-bold btn-press disabled:opacity-50">Stop Campaign</button>
                </div>
              </motion.div>
            </motion.div>
          )}
      ...
      <div className="px-5 pt-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[14px] text-muted-foreground">{t('campaigns.activeCampaigns', { count: externalStats?.activeCount ?? 3 })}</p>
          </div>
          <button
        ...
        <div className="flex gap-3 mt-4 overflow-x-auto pb-1 -mx-1 px-1 md:justify-center">
          {isLoading ? (
            ...
            <>
              {[
                { icon: '💰', value: externalStats ? `SAR ${externalStats.totalBudget.toLocaleString()}` : 'SAR 10,500', label: t('campaigns.totalBudget'), bg: 'bg-purple-soft' },
                { icon: '👁️', value: externalStats ? (externalStats.totalReach >= 1000 ? `${(externalStats.totalReach / 1000).toFixed(1)}K` : String(externalStats.totalReach)) : '124.4K', label: t('campaigns.totalReach'), bg: 'bg-green-soft' },
                { icon: '📈', value: externalStats ? `${externalStats.avgRoi}x` : '2.5x', label: t('campaigns.avgRoi'), bg: 'bg-green-soft' },
              ].map((s, i) => (
          ...
          <div className="mt-4">
            ...
            <div className="mt-3 space-y-3">
              {isLoading ? (
                ...
                (displayCampaigns[filter] ?? []).map((c: any, i: number) => (
                  <motion.div
                    key={i}
                  ...
                  >
                    <div className="p-4">
                      ...
                      <div className="flex gap-1 mt-2">
                        {c.logos.map((Logo: any, j: number) => <Logo key={j} size={16} />)}
                      </div>
