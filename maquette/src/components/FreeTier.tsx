...
// ── Trial helpers ──
const TRIAL_START_KEY = 'speeda_trial_start';
const TRIAL_DAYS = 14;
...
const getTrialDaysRemaining = (): number => {
  ...
  const elapsed = Math.floor((Date.now() - parseInt(start)) / (1000 * 60 * 60 * 24));
  return Math.max(0, TRIAL_DAYS - elapsed);
};

const isTrialExpired = (): boolean => getTrialDaysRemaining() <= 0;
...
export const FreeTierProvider = ({ children }: { children: React.ReactNode }) => {
  const [messagesUsed, setMessagesUsed] = useState(2);
