...
import { ChevronLeft, ChevronRight, User, Shield, CreditCard, Coins, Globe2, Wifi, Brain, Bell, BellDot, BellRing, Languages, HelpCircle, MessageCircle, Sparkles, Info, Gift, MoreVertical, Rss, ExternalLink, Pause, Play, Trash2 } from 'lucide-react';
...
export const SettingsScreen = ({ onBack, onNavigate }: SettingsScreenProps) => {
  ...
  const toggleAuto = (idx: number) => {
    // Auto-boost (index 0) and auto-adjust budgets (index 4) are Coming Soon
    ...
    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="bg-background min-h-screen pb-24">
      <div className="px-5 pt-6">
        ...
        <Section title={t('settings.account')}>
          <Row label={t('settings.profile')} value="Malek Zlitni" icon={User} onClick={() => onNavigate?.('profile')} />
          ...
          <div className="px-4 py-3.5 border-b border-border-light">
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[10px] font-bold text-primary-foreground gradient-btn px-2 py-0.5 rounded-md">Pro Pack</span>
              <span className="text-[13px] font-bold text-foreground">1,449 SAR/{i18n.language === 'ar' ? 'شهر' : i18n.language === 'fr' ? 'mois' : 'month'}</span>
            </div>
        ...
        <Section title={t('settings.tokensSection')}>
          <button onClick={() => onNavigate?.('tokens')} className="w-full px-4 py-3.5 text-left">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[14px] font-medium text-foreground">{t('settings.tokensRemaining', { count: 142 })}</p>
                <div className="h-1.5 rounded-full bg-muted mt-2 w-40 overflow-hidden">
                  <div className="w-[28%] h-full gradient-btn rounded-full" />
                </div>
              </div>
