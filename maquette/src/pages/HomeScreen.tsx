...
const TappableMosRing = ({ size, onTap }: { size: number; onTap: () => void }) => {
  ...
  return (
    <div className="relative">
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
      >
        <MosScoreRing score={74} size={size} />
      </motion.button>
      <AnimatePresence>
        {showTooltip && (
          <motion.div
...
export const HomeScreen = ({ onNavigate, pendingActionCardId, onClearPendingAction }: HomeScreenProps) => {
  ...
  const tokenLow = tokenCount < 100;
  const hasNegativeReview = true;

  const initialCards: StrategyCard[] = [
    { id: 1, priority: t('strategy.critical'), color: 'bg-red-accent', title: t('strategy.postPeak'), desc: t('strategy.postPeakDesc'), impact: '+40% reach', impactIcon: '📈', nav: 'create' },
    { id: 2, priority: t('strategy.high'), color: 'bg-orange-accent', title: t('strategy.respondReviews'), desc: t('strategy.respondReviewsDesc'), impact: '+15% trust score', impactIcon: '📈', nav: 'chat-engagement' },
    { id: 3, priority: t('strategy.recommended'), color: 'bg-purple', title: t('strategy.boostPost'), desc: t('strategy.boostPostDesc'), impact: 'Est. 50K reach', impactIcon: '📈', nav: 'campaigns' },
    { id: 4, priority: t('strategy.recommended'), color: 'bg-brand-blue', title: 'Share a customer review', desc: 'Khalid left a 5-star review on Google. Sharing positive reviews as posts increases trust by 35%.', impact: 'Est. +800 reach · +35% trust score', impactIcon: '📈', nav: 'create' },
    { id: 5, priority: t('strategy.recommended'), color: 'bg-brand-blue', title: 'Update your Google Business photos', desc: 'Your last Google Business photo was uploaded 3 weeks ago. Fresh photos improve local search ranking.', impact: 'Est. +15% local visibility', impactIcon: '📈', nav: 'settings' },
  ];

  const [activeCards, setActiveCards] = useState<StrategyCard[]>(initialCards);
        ...
        <div className="px-8 pt-6">
          ...
          <div className="mb-4 bg-red-accent/10 border border-red-accent/30 rounded-2xl p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-[12px] text-muted-foreground">X (Twitter) needs to be reconnected</p>
            </div>
            <button onClick={() => onNavigate('settings')} className="text-[12px] font-bold text-brand-blue bg-brand-blue/10 px-3 py-1.5 rounded-lg whitespace-nowrap">Reconnect Now</button>
          </div>
          ...
          <div className="grid grid-cols-5 gap-6">
            ...
            <div className="col-span-2 space-y-4">
              <div className="bg-card rounded-3xl border border-border-light p-6 shadow-card desktop-hover flex flex-col items-center">
                <TappableMosRing size={120} onTap={() => onNavigate('mosScore')} />
          ...
          <div className="mt-6 grid grid-cols-3 gap-6">
            ...
            <button onClick={() => onNavigate('social')} className="w-full bg-card rounded-2xl p-5 border border-border-light shadow-card flex flex-col justify-between desktop-hover text-start">
              <div>
                ...
                <p className="text-[12px] text-muted-foreground mt-1">{t('home.socialMediaDesc')}</p>
              </div>
              <div className="flex gap-1 mt-2">
                <InstagramLogo size={14} /><TikTokLogo size={14} /><SnapchatLogo size={14} /><FacebookLogo size={14} /><XLogo size={14} />
              </div>
            </button>
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="bg-background min-h-screen pb-24">
      ...
      <div className="px-5 pt-6">
        ...
        <div className="flex justify-between items-start">
          <div>
            <p className="text-[14px] text-muted-foreground">{t('home.greeting')},</p>
            <h1 className="text-[28px] font-extrabold tracking-[-0.02em] text-foreground">{t('home.name')}</h1>
        ...
        <div className="flex justify-end -mt-2">
          <TappableMosRing size={64} onTap={() => onNavigate('mosScore')} />
        </div>
      ...
      <SalesAgent onNavigate={onNavigate} currentPlan="starter" tokensUsed={420} tokensLimit={500} postsThisMonth={11} />
    </motion.div>
  );
};
