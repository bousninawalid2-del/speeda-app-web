...
export const TokensScreen = ({ onBack, scrollToPacks }: TokensScreenProps) => {
        ...
        <div className="px-5 pt-6">
          ...
          <div className="bg-card rounded-2xl border border-border-light p-5">
            <div className="flex items-baseline gap-2">
              <span className="text-[32px] font-extrabold text-brand-blue tracking-[-0.02em]">✦ 342</span>
              <span className="text-[18px] text-muted-foreground">{t('tokens.of', { total: 800 })}</span>
            </div>
            <div className="h-3 rounded-full bg-muted mt-3 overflow-hidden">
              <motion.div initial={{ width: 0 }} animate={{ width: '42.8%' }} transition={{ duration: 0.8 }} className="h-full gradient-btn rounded-full" />
            </div>
            ...
            {tokenPacks.map((pack, i) => (
              <div key={i} className="bg-card rounded-2xl border border-border-light p-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setBuyingPack(i)} className={`h-10 px-5 rounded-xl text-[13px] font-bold btn-press ${pack.badge === 'popular' ? 'gradient-btn text-primary-foreground shadow-btn' : 'border border-border text-foreground'}`}>
                    {t('tokens.buy')}
                  </button>
                </div>
              </div>
          ...
          <div className="bg-card rounded-2xl border border-border-light p-4 mb-5">
            <div className="space-y-3">
              {agentSummary.map(({ agent, tokens, percent }) => (
                <div key={agent}>
                  ...
                  <div className="h-2 rounded-full bg-muted overflow-hidden">
                    <motion.div
                      ...
                      style={{ backgroundColor: agentColors[agent].bar }}
                    />
                  </div>
                </div>
              ))}
            ...
            <p className="text-[12px] text-muted-foreground mt-3 pt-3 border-t border-border-light"
              dangerouslySetInnerHTML={{ __html: t('tokens.totalUsed', { used: 458, total: 800 }) }}
            />
          </div>
          ...
          <div className="bg-card rounded-2xl border border-border-light overflow-hidden">
            {usageHistory.map((item, i) => {
              const ac = agentColors[item.agent];
              return (
      ...
      <AnimatePresence>
        {buyingPack !== null && (
          <PaymentFlow
            ...
            successDetail={`New balance: ${342 + tokenPacks[buyingPack].amount} tokens`}
