...
export const ProfileScreen = ({ onBack, onNavigate }: ProfileScreenProps) => {
  const { t } = useTranslation();
  const [form, setForm] = useState({
    businessName: "Malek's Kitchen",
    ownerName: 'Malek Zlitni',
  ...
  // User status
  const [accountStatus] = useState<'active' | 'pending' | 'unverified'>('active');
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="bg-background min-h-screen pb-8">
      <div className="px-5 pt-6">
        ...
        <button className="w-full h-[56px] rounded-2xl gradient-btn text-primary-foreground font-bold text-[15px] shadow-btn btn-press mt-8">
