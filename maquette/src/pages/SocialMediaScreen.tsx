...
const initialAccounts = [
  { Logo: InstagramLogo, name: 'Instagram', followers: '12.4K', connected: true, health: 'healthy' as const },
  { Logo: TikTokLogo, name: 'TikTok', followers: '8.2K', connected: true, health: 'healthy' as const },
  { Logo: SnapchatLogo, name: 'Snapchat', followers: '3.1K', connected: true, note: 'Limited features — Stories only', health: 'warning' as const },
  { Logo: FacebookLogo, name: 'Facebook', followers: '5.6K', connected: true, health: 'healthy' as const },
  { Logo: XLogo, name: 'X', followers: '2.8K', connected: true, health: 'error' as const },
  { Logo: YouTubeLogo, name: 'YouTube', followers: '1.5K', connected: true, health: 'healthy' as const },
  { Logo: GoogleLogo, name: 'Google Biz', followers: '245 reviews', connected: true, health: 'healthy' as const },
  { Logo: LinkedInLogo, name: 'LinkedIn', followers: '980', connected: false, health: 'none' as const },
  { Logo: PinterestLogo, name: 'Pinterest', followers: '—', connected: false, health: 'none' as const },
  { Logo: ThreadsLogo, name: 'Threads', followers: '—', connected: false, health: 'none' as const },
];
...
export const SocialMediaScreen = ({ onBack }: { onBack: () => void }) => {
    ...
    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="bg-background min-h-screen pb-24">
      <div className="px-5 pt-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <button className="h-8 px-3 rounded-xl bg-brand-blue text-primary-foreground text-[12px] font-bold flex items-center gap-1"><Plus size={14} />Connect</button>
        </div>
        ...
        <div className="gradient-hero rounded-2xl p-4 grid grid-cols-3 gap-2 text-center">
          {[{ v: '34.5K', l: 'Total Followers' }, { v: `${connectedCount}/${accounts.length}`, l: 'Platforms' }, { v: '5', l: 'Scheduled' }].map((s, i) => (
            <div key={i}>
            ...
            {accounts.map((a, i) => (
              ...
              <div className="relative">
                ...
                <div className="flex-1">
                  ...
                  {'note' in a && (a as any).note && <p className="text-[10px] text-muted-foreground">{(a as any).note}</p>}
                  {a.health === 'error' && a.connected && (
                    <div className="flex items-center gap-1 mt-1">
                      <span className="text-[10px] text-red-accent font-semibold">⚠️ Connection issue</span>
                      <button className="text-[10px] text-brand-blue font-semibold">Reconnect</button>
                    </div>
                  )}
                  {a.health === 'warning' && a.connected && (
                    <p className="text-[10px] text-orange-accent font-medium mt-0.5">⚠️ Token expiring soon</p>
                  )}
                </div>
                {a.connected ? (
                  ...
                  <button onClick={() => setConnectingPlatform({ name: a.name, Logo: a.Logo })} className="text-[11px] font-semibold text-brand-blue">Connect</button>
                )}
              </div>
            ))}
          </div>
      ...
      <AnimatePresence>
        {connectingPlatform && (
          <PlatformConnectFlow
            ...
            onComplete={() => {
              setAccounts(accs => accs.map(a => a.name === connectingPlatform.name ? { ...a, connected: true } : a));
        ...
        {disconnectPlatform && (
          <DisconnectDialog
            ...
            onConfirm={() => {
              setAccounts(accs => accs.map(a => a.name === disconnectPlatform ? { ...a, connected: false } : a));
